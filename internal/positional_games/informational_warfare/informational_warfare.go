...
func (g *InformationalWarfare) Solve() {
	...
	fmt.Printf("Final opinions:\n %.3v\nRes trust matrix:\n %.3v\n",
		mat.Formatted(finalOpinions), mat.Formatted(resTrustMatrix))
	...
	player2Influence := rand.Intn(player1Influence+diff-player1Influence+diff) + player1Influence - diff
	...
	targetS := fmt.Sprintf("%.3f * u^2 + %.3f * u + %.3f * v + %.3f * u * v + %.3f v^2",
		-float64(g.d)*rF*rF-float64(g.gS)/2,
		float64(g.c)*rF,
		float64(g.c)*rS,
		-float64(2)*float64(g.c)*rF*rS,
		-float64(g.d)*rS*rS,
	)
	...
	targetFDiffU := fmt.Sprintf("%.3f * u + %.3f * v + %.3f",
		-2*float64(g.d)*rF*rF-float64(g.gS),
		-2*float64(g.b)*rS*rF,
		float64(g.a)*rF,
	)
	...
	v := (float64(g.a)*rF*-float64(2)*float64(g.c)*rF*rS - -2*float64(g.d)*rF*rF - float64(g.gS)*float64(g.c)*rS) / (-2*float64(g.d)*rF*rF - float64(g.gS)*-2*float64(g.d)*rS*rS - -2*float64(g.b)*rS*rF*-float64(2)*float64(g.c)*rF*rS)
	u := (2*float64(g.b)*rS*rF*v - float64(g.a)*rF) / (-2*float64(g.d)*rF*rF - float64(g.gS))
...
func (g *InformationalWarfare) computeFinalOpinions(trustMatrix *mat.Dense, initialOpinions *mat.VecDense, epsilon float64) (*mat.VecDense, *mat.Dense) {
